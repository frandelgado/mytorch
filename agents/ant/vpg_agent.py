...
class VPGAgent(Agent):
    ...
    def __init__(self, state_space: int, action_space: int, hidden=50, lr=1e-3, gamma=0.9):
        ...
        shared_dim_output = 20
        shared_layers = [
            Sigmoid(state_space, hidden),
            Sigmoid(hidden, shared_dim_output),
        ]
        ...
        for action in range(action_space):
            self.action_heads.append(
                Net(
                    # TODO missing discretized action space? or a unidimentional gaussian?
                    layers=shared_layers + [Softmax(shared_dim_output, None)],
                    optimizer=Adam(),
                    lr=lr,
                )
            )
